Feature: Cube Rotation and Solving Substeps
  ...
  Rule: The user can manually rotate the cube along any axis
    ...
    Scenario: Rotate cube around the x-axis (tip forward)
      Given the cube is displayed with F face at the front
      When the user applies an x rotation
      Then the cube display updates so the U face is now at the front
      And the cube state (scramble + moves) is unchanged

    Scenario: Rotate cube around the x-axis inverse (tip backward)
      Given the cube is displayed with F face at the front
      When the user applies an x' rotation
      Then the cube display updates so the D face is now at the front
      And the cube state (scramble + moves) is unchanged

    Scenario: Rotate cube around the y-axis (turn right)
      Given the cube is displayed with F face at the front
      When the user applies a y rotation
      Then the cube display updates so the R face is now at the front
      And the cube state (scramble + moves) is unchanged

    Scenario: Rotate cube around the y-axis inverse (turn left)
      Given the cube is displayed with F face at the front
      When the user applies a y' rotation
      Then the cube display updates so the L face is now at the front
      And the cube state (scramble + moves) is unchanged

    Scenario: Rotate cube around the z-axis (roll clockwise)
      Given the cube is displayed with F face at the front
      When the user applies a z rotation
      Then the cube display updates so U face moves to the right side
      And the cube state (scramble + moves) is unchanged

    Scenario: Rotate cube around the z-axis inverse (roll counter-clockwise)
      Given the cube is displayed with F face at the front
      When the user applies a z' rotation
      Then the cube display updates so U face moves to the left side
      And the cube state (scramble + moves) is unchanged

    Scenario: Cube rotation persists across move inputs
      Given the user has applied a y rotation so R is at the front
      When the user inputs a move sequence
      Then the cube display continues to show R at the front after each move

    Scenario: Cube rotation resets when a new substep is selected
      Given the user has manually rotated the cube to an arbitrary orientation
  ...
  Rule: Selecting an EO substep sets the canonical front face for that axis
    ...
    Scenario: Selecting eofb sets front face to F
      ...
      Then the cube display shows the F face at the front
      And the substep label "EO-FB" is highlighted as active

    Scenario: Selecting eorl sets front face to R
      Given the EO step is active
      When the user selects substep "eorl"
      Then the cube display shows the R face at the front
      And the substep label "EO-RL" is highlighted as active

    Scenario: Selecting eoud sets front face to U
      Given the EO step is active
      When the user selects substep "eoud"
      Then the cube display shows the U face at the front
      And the substep label "EO-UD" is highlighted as active

    Scenario: Substep orientation is reflected in the solution display
      ...
      Then the step-by-step display appends the line:
        """
        {moves} // EO-RL ({stepCount}/{runningTotal})
        """
    ...
    Scenario: Switching between EO substeps updates the cube orientation immediately
      ...
      Then the cube display immediately shows R at the front
      When the user switches to substep "eoud"
      Then the cube display immediately shows U at the front

    Scenario: EO substep selection is preserved after entering moves
      Given the EO step is active and substep "eorl" is selected
      When the user inputs several moves
      Then the cube display continues to show R at the front
  ...
  Rule: Selecting a DR substep sets the canonical front face for that axis
    ...
    Scenario: Selecting drud keeps the default orientation (F face at front)
      ...
      Then the cube display shows the F face at the front
      And the substep label "DR-UD" is highlighted as active

    Scenario: Selecting drrl sets front face to R
      Given the DR step is active
      When the user selects substep "drrl"
      Then the cube display shows the R face at the front
      And the substep label "DR-RL" is highlighted as active

    Scenario: Selecting drfb sets front face to F
      Given the DR step is active
      When the user selects substep "drfb"
      Then the cube display shows the F face at the front
      And the substep label "DR-FB" is highlighted as active
